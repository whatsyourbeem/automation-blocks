...
#   3. 조건(1): 기준 값보다 큰 값을 찾을지 작은 값을 찾을지 입력
#       - "bigger than" 혹은 "smaller than" 으로 입력해야 함
#   4. 조건(2): 기준 값 (숫자)
...
def main():
    ...
    print("'" + targetValue + "' 보다 ", end='')
    print("큰 값을 가진 " if condition=="bigger than" else "작은 값을 가진", end='')
    ...
    if searchMode=='row':
        for i in range(1, last_column+1):
            ...
            # 비교 대상 값이 공백이거나, 숫자가 아니라면 PASS
            if value==None or (type(value)!=int and type(value)!=float):
                continue
            if (((condition=="bigger than") and (value>float(targetValue))) or
                ((condition=="smaller than") and (value<float(targetValue)))):
...
def check_validity(argv):
    ...
    if not argv[3].isalpha() and not argv[3].isdigit():
        ...
        isPassed = False
    # 조건 정합성 검사
    if (argv[4]!="bigger than") and (argv[4]!="smaller than"):
        print("4번째 조건이 올바르지 않습니다. 'bigger than' 혹은 'smaller than'으로 입력해주세요.")
        isPassed = False
    return isPassed
